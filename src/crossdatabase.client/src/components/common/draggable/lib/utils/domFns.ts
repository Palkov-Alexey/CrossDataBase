...
export function addEvent(el: Node | null, event: string, handler: () => void, inputOptions?: object): void {
    if (!el) return;
...
export function removeEvent(el: ?Node, event: string, handler: () => void, inputOptions?: object): void {
    if (!el) return;
